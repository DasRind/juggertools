...
import { ToastAction, ToastService, Toast } from '@juggertools/ui-angular';
import {
  DrawToolEvent,
  DrawToolId,
  DrawToolRegistry,
...
interface QuickPlayerDraft {
  name: string;
  color: string;
  pompfenId: PompfenId | null;
}

interface QuickPlayer extends QuickPlayerDraft {
  id: string;
  tokenId: string;
}
...
const DEMO_TEAM_PLAYERS: Record<
  TeamSide,
  readonly { name: string; pompfenId: PompfenId | null }[]
> = {
  left: [
    { name: 'Lena Blitz', pompfenId: 'chain' },
    { name: 'Mara Schild', pompfenId: 'shield' },
    { name: 'Tommy Sprint', pompfenId: 'sword' },
    { name: 'Kai Taktik', pompfenId: 'staff' },
    { name: 'Nico Backup', pompfenId: 'reserve' },
  ],
  right: [
    { name: 'Sven Schnell', pompfenId: 'q-tip' },
    { name: 'Ella Block', pompfenId: 'shield' },
    { name: 'Riko Fokus', pompfenId: 'staff' },
    { name: 'Mira Tempo', pompfenId: 'dual' },
    { name: 'Jule Support', pompfenId: 'reserve' },
  ],
};
...
export class TacticsStateService {
  ...
  readonly lineColors = this.penColors;
  readonly selectedLineColor = signal<string>(this.lineColors[0]);
  ...
  readonly selectedConeColor = signal<string>(this.coneColors[0]);
  ...
  readonly showQuickAddPanel = signal<boolean>(false);
  ...
  private resolveTeamSide(teamId: string | undefined): TeamSide | null {
    if (!teamId) {
      return null;
    }
  ...
  private createInitialTeamConfig(side: TeamSide): TeamConfig {
    ...
    const playerPrefix = side === 'left' ? 'left' : 'right';
    return {
      id: teamId,
      ...
      logoDataUrl: null,
      players: DEMO_TEAM_PLAYERS[side].map((player, index) => ({
        id: `${playerPrefix}-${index + 1}`,
        name: player.name,
        pompfenId: player.pompfenId,
      })),
    };
  }
  ...
  private getQuickPlayer(playerId: string | undefined): QuickPlayer | undefined {
    ...
    return this.quickPlayersState()[playerId];
  }

  private updateQuickPlayer(
    playerId: string,
    patch: Partial<Omit<QuickPlayer, 'id'>>
  ): void {
    this.quickPlayersState.update((players) => {
  ...
  private removeQuickPlayer(playerId: string | undefined): void {
    if (!playerId) {
      return;
    }
    this.quickPlayersState.update((players) => {
    ...
    this.sceneSnapshot.update((snapshot) => {
      const filteredTokens = snapshot.scene.tokens
        ...
        .map((token) => {
          ...
          const quick = quickPlayers[token.playerId];
          if (!quick) {
            return token;
          }
      ...
      this.pruneQuickPlayers(filteredTokens);
  ...
  private readonly applySceneUpdate = (
    ...
    this.sceneSnapshot.update((snapshot) => {
      ...
      this.pruneQuickPlayers(withTimestamp.scene.tokens);
  ...
  private readonly toolDisplayOrder = new Map<DrawToolId, number>([
    ['select', 0],
    ['pen', 1],
    ['eraser', 2],
    ['line', 3],
  ...
  readonly tools = this.toolRegistry
    .listTools()
    .slice()
    .sort((a, b) => {
      const rankA = this.toolDisplayOrder.get(a.id) ?? Number.MAX_SAFE_INTEGER;
      const rankB = this.toolDisplayOrder.get(b.id) ?? Number.MAX_SAFE_INTEGER;
      if (rankA === rankB) {
  ...
  readonly toolStatus = computed<ToolStatus | null>(() => {
    ...
    if (tool === 'cone') {
      return {
        kind: 'cone',
        label: 'Zone',
        color: this.selectedConeColor(),
        palette: this.coneColors,
      };
    }
    return null;
  });
  ...
  readonly openToolColorMenu = computed<
    ('pen' | 'line' | 'arrow' | 'cone') | null
  >(() => {
  ...
  updateQuickAddName(name: string): void {
    this.quickAddDraft.update((draft) => ({ ...draft, name }));
  }

  updateQuickAddColor(color: string): void {
    const normalized = this.normalizeColor(color) ?? QUICK_DEFAULT_COLOR;
    this.quickAddDraft.update((draft) => ({ ...draft, color: normalized }));
  }

  updateQuickAddPompfen(pompfenId: PompfenId | null): void {
    this.quickAddDraft.update((draft) => ({
      ...draft,
      pompfenId: this.ensurePompfenId(pompfenId),
    }));
  }

  canSubmitQuickAdd(): boolean {
    return this.quickAddDraft().name.trim().length > 0;
  }

  createQuickPlayer(): void {
    const draft = this.quickAddDraft();
    const name = draft.name.trim();
    if (!name) {
      this.toastService.show({
        message: 'Name für neuen Spieler fehlt',
        intent: 'warning',
    ...
    const color = this.normalizeColor(draft.color) ?? QUICK_DEFAULT_COLOR;
    const pompfenId = this.ensurePompfenId(draft.pompfenId);
    const playerId = this.createQuickPlayerId();
    const tokenId = this.createTokenId(playerId);

    const field = this.scene().scene.field;
    const spawnX = field.width * 0.62;
    const spawnY = field.height * 0.52;
    ...
    this.selectedTarget.set({ type: 'token', id: tokenId });
  ...
  constructor() {
    ...
    this.applyLineConfiguration();
    this.applyArrowConfiguration();
    this.applyConeConfiguration();
  ...
  toggleColorMenu(
    context: 'selection' | 'pen' | 'line' | 'arrow' | 'cone'
  ): void {
  ...
  drawingKindLabel(kind: Drawing['kind'] | undefined): string {
    switch (kind) {
      case 'line':
        return 'Linie';
      case 'pen':
  ...
  applyToolColor(tool: 'pen' | 'line' | 'arrow' | 'cone', color: string): void {
    const normalized = this.normalizeColor(color);
    if (!normalized) {
      return;
    }
    let effectiveColor = normalized;
    if (tool === 'pen') {
      if (this.colorsEqual(this.selectedPenColor(), effectiveColor)) {
        return;
      }
      this.selectedPenColor.set(effectiveColor);
      this.applyPenConfiguration();
    } else if (tool === 'line') {
      if (this.colorsEqual(this.selectedLineColor(), effectiveColor)) {
        return;
      }
      this.selectedLineColor.set(effectiveColor);
      this.applyLineConfiguration();
    } else {
      if (tool === 'arrow') {
        if (this.colorsEqual(this.selectedArrowColor(), effectiveColor)) {
          return;
        }
        this.selectedArrowColor.set(effectiveColor);
        this.applyArrowConfiguration();
      } else {
        effectiveColor = this.ensureConeColor(normalized);
        if (this.colorsEqual(this.selectedConeColor(), effectiveColor)) {
          return;
        }
        this.selectedConeColor.set(effectiveColor);
        this.applyConeConfiguration();
      }
    }
    ...
    if (drawing && drawing.kind === tool) {
      this.updateSelectedColor(effectiveColor);
    }
  ...
  private resetSelectionState(): void {
    ...
    this.isDraggingToken.set(false);
    this.isDraggingDrawing.set(false);
    if (this.colorMenuContext()?.kind === 'selection') {
      this.colorMenuContext.set(null);
    }
  }
  ...
  private applyLineConfiguration(): void {
    this.toolRegistry.configureTool('line', {
      color: this.selectedLineColor(),
      width: LINE_BASE_WIDTH * this.strokeWidthFactor(),
    });
  }
  ...
  private applyConeConfiguration(): void {
    this.toolRegistry.configureTool('cone', {
      color: this.selectedConeColor(),
    });
  }
  ...
  toggleQuickAddPanel(): void {
    this.showQuickAddPanel.update((value) => !value);
  }
  ...
  playerTokenId(player: { id: string }): string | undefined {
  ...
  private updateSelectedColor(color: string): void {
    ...
    if (target.type === 'token') {
      ...
      this.applySceneUpdate((state) => ({
        ...
        scene: {
          ...
          tokens: state.scene.tokens.map((token) => {
            ...
            affectedPlayerId = token.playerId;
            return {
              ...token,
              color: normalized,
            };
      ...
      if (affectedPlayerId) {
        const quick = this.getQuickPlayer(affectedPlayerId);
        if (quick) {
          this.updateQuickPlayer(affectedPlayerId, { color: normalized });
        }
      }
      this.engine?.draw();
      return;
  ...
  updateSelectedPlayerName(name: string): void {
    const target = this.normalizeTarget(this.selectedTarget());
    ...
    const token = this.scene().scene.tokens.find(
      (entry) => entry.id === target.id
    );
    if (!token) {
      return;
    ...
    if (playerId) {
      const quick = this.getQuickPlayer(playerId);
      if (quick) {
        if (quick.name === trimmed) {
          return;
        }
        this.updateQuickPlayer(playerId, { name: trimmed });
        this.applySceneUpdate((state) => ({
          ...state,
          scene: {
            ...state.scene,
            tokens: state.scene.tokens.map((entry) =>
              entry.id === token.id ? { ...entry, label: trimmed } : entry
            ),
          },
        }));
        return;
      ...
      const teamSide = this.resolveTeamSide(token.teamId);
      if (teamSide) {
        const current = this.getTeamState(teamSide).players.find(
          (player) => player.id === playerId
        );
        if (current?.name === trimmed) {
          return;
        }
        this.updatePlayerName(teamSide, playerId, trimmed);
        this.persistTeamState(teamSide);
        return;
      }
    }
    this.applySceneUpdate((state) => ({
  ...
  updateSelectedPlayerPompfen(pompfenId: PompfenId | null): void {
    const target = this.normalizeTarget(this.selectedTarget());
    if (!target || target.type !== 'token') {
      return;
    }
    ...
    const normalized = this.ensurePompfenId(pompfenId);
    const quick = this.getQuickPlayer(token.playerId);
    if (quick) {
      if (quick.pompfenId === normalized) {
        return;
      }
      this.updateQuickPlayer(token.playerId, { pompfenId: normalized });
      return;
    }
    const teamSide = this.resolveTeamSide(token.teamId);
    if (!teamSide) {
    ...
    this.assignPompfenToPlayer(teamSide, token.playerId, normalized);
  }

  updateSelectedPlayerColor(color: string): void {
    this.updateSelectedColor(color);
  }
  ...
  private ensureConeColor(color: string): string {
  ...
  private pruneQuickPlayers(tokens: readonly Token[]): void {
    const keep = new Set(
      tokens
        .map((token) => token.playerId)
        .filter((value): value is string => Boolean(value))
    );
    const current = this.quickPlayersState();
    let mutated = false;
    const next: Record<string, QuickPlayer> = {};
    Object.entries(current).forEach(([id, player]) => {
      if (!keep.has(id)) {
        mutated = true;
        return;
      }
      const token = tokens.find((token) => token.playerId === id);
      if (token && token.id !== player.tokenId) {
        mutated = true;
        next[id] = { ...player, tokenId: token.id };
      } else {
        next[id] = player;
      }
    });
    if (mutated) {
      this.quickPlayersState.set(next);
    }
  }
